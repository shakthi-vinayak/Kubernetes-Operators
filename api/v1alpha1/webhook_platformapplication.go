...
// These use the generic admission.Defaulter and admission.Validator
...
// PlatformApplicationDefaulter implements admission.Defaulter for PlatformApplication.
type PlatformApplicationDefaulter struct{}

// Default sets sensible defaults for fields that are not explicitly configured.
// Defaulting webhooks run before validation, filling in optional fields
...
// PlatformApplicationValidator implements admission.Validator for PlatformApplication.
...
// Delete validation is typically empty unless there are external resources
// that need to be verified before deletion.
func (v *PlatformApplicationValidator) ValidateDelete(_ context.Context, _ *PlatformApplication) (admission.Warnings, error) {
	return nil, nil
}

// validatePlatformApplication performs cross-field validation that cannot
// be expressed via CRD OpenAPI schema markers alone.
